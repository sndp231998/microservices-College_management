...
//	create product
	@PostMapping("/user/{userId}/category/{borrowId}/borrows")
	public ResponseEntity<BorrowDto> createBorrow(@RequestBody BorrowDto borrowDto, @PathVariable Integer userId,
			@PathVariable Integer bookId) {
		BorrowDto createBorrow = this.borrowService.createBorrow(borrowDto, userId, bookId);
	...
	// get by user
		...
		public ResponseEntity<List<BorrowDto>> getBorrowsByUser(@PathVariable Integer userId) {
			...
			List<BorrowDto> borrows = this.borrowService.getBorrowsByUser(userId);
